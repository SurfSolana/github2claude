...
export async function executeCodeAnalysis(repoUrl) {
    const { username, repoName } = extractRepoInfo(repoUrl);
    const git = simpleGit();
    ...
    const outputDirName = `${username}__${repoName}`;
    ...
    try {
        ...
        await fs.mkdir(join(cwd, config.tempDir), { recursive: true });
        ...
        await fs.mkdir(versionedOutputPath, { recursive: true });
        ...
        try {
            ...
            await fs.rm(join(cwd, config.tempDir), { recursive: true, force: true });
...
export { CodeAnalyzer, MarkdownGenerator };